...
export default function Sidebar({ categories }) {
  ...
  return (
    <div className="w-full lg:w-1/3 px-2">
      {/* <!-- divider --> */}
      <div className="border border-dotted mt-4 visible lg:invisible lg:mt-0"></div>
            ...
            <li className="p-1 py-4 border-b border-t border-white hover:border-gray-200 transition duration-300" key={name}>
              <Link href={"/category/[category]"} as={`/category/${name}`}>
                <a className="flex items-center text-gray-600 cursor-pointer">
                  ...
                  <span>{name}</span>
                  <span className="text-gray-500 ml-auto">{count} articles</span>
      ...
      <div className="opacity-25 my-2 lg:my-4">
        ...
        <p className="text-gray-600 pt-2 text-xs text-center">
          or, follow the <Link href={`/feed.xml`}><a>RSS feed</a></Link>!
        </p>
      </div>

      {/* <!-- divider --> */}
      <div className="border border-dotted invisible lg:visible"></div>
    </div>
  );
}
